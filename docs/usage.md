...
### 2.0 初回セットアップウィザード（画面）

初めてブラウザでOCR Crafterを開くと、**初回セットアップウィザード**が自動表示されます（7ステップ・数分で完了）。

1. ようこそ → 2. プロジェクト保存先（書き込み確認・変更方法の案内） → 3. OCRエンジン確認（Tesseract/PaddleOCR。未導入でも続行可） → 4. GPU確認（GPU名・CUDA可否。CPUでも実行可） → 5. Python環境確認 → 6. バックアップ推奨設定（metadata=毎日 / full=毎週） → 7. 完了（新規プロジェクト等のショートカット）

- 完了するとブラウザ（localStorage）へ記録され、次回以降は表示されません
- 途中で右上×から中断できます（確認あり。次回起動時に再表示）。**Escキーでは閉じません**
- もう一度実行したい場合は「**運用 > システム状態**」の「**セットアップを再実行**」を押してください

### 2.1 新規プロジェクトの作成（テンプレート選択）

ダッシュボード右上の「**新規プロジェクト**」から、用途別テンプレートを選んで作成できます。

1. **テンプレート選択**: 標準プロジェクト / 英数字OCR（型式・シリアル番号）/ 日本語OCR（銘板・帳票）/ 銘板OCR / 手書きOCR（筆記体）/ OCR＋YOLO（領域検出つき）の6種。カード下部で適用される設定を確認できます
2. **プロジェクト基本情報**: プロジェクト名を入力
3. **設定内容の確認** → **作成**

テンプレートは**初期値を設定するだけ**で、作成後はすべての設定を通常どおり変更できます。標準プロジェクトを選べば従来の新規作成と同等です。作成元テンプレートはダッシュボードの「現在のプロジェクト」で確認できます（既存プロジェクトは「記録なし」表示）。

### 2.2 モデル開発レポートの生成

「**運用 > レポート**」から、学習・評価・比較・リリース情報をまとめたレポートをMarkdown/PDFで自動生成できます。

1. 種別を選択: 単一モデル / モデル比較（2件以上）/ プロジェクト総括
2. 出力形式（Markdown・PDF・両方）と画像掲載有無を選択し「レポートを生成」
3. 生成はバックグラウンドJob（進捗はジョブ管理）。完了後にレポート履歴からダウンロード・再生成・削除（確認つき）

保存先は `data/reports/<project_id>/`、Report IDは RPT-0001形式。記録のない項目は「記録なし」と表示され、**生成時に外部Webサービスへの送信は行いません**（PDFはmatplotlibでローカル生成・日本語フォント対応）。

以下はコマンドラインでの環境構築手順です。

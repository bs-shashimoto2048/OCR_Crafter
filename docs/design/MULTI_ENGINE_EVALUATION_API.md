...
**Common Metric Calculator実装中**（Feature [#65](https://github.com/bs-shashimoto2048/OCR_Crafter/issues/65)、19章のPhase 2〜3に相当。**Implemented, PR review pending**。`src/app/services/evaluation_metrics.py`を新設し、`normalize_compare`/`levenshtein_ops`（既存`ocr_evaluation.py`と出力一致をテストで直接検証、logger名の副作用を避けるため独立実装として用意）、`calculate_sample_metrics`/`calculate_evaluation_metrics`/`aggregate_confusions`（Common Evaluation Schemaを出力に利用）を実装。CERは既存仕様どおりマイクロ平均、character_accuracyは負値許容、confusionは`from→expected`/`to→predicted`変換。`sample_count`重複は`metrics.sample_count`をCanonicalとする方針を確定（Result全体は本Issueでは生成しない）。既存Tesseract評価（`ocr_evaluation.py`）への配線は見送り、Tesseract Predictor Adapter Issueへ持ち越し。詳細は[docs/workitems/trocr/COMMON_EVALUATION_METRICS_65.md](../workitems/trocr/COMMON_EVALUATION_METRICS_65.md)参照。次の実装対象はEvaluation Dispatcher / Runner。Issue分割案（19章）は確定版として整理済み。Dispatcher/Predictor/API接続は本ドキュメント時点では未着手。

...
Feature [#40](https://github.com/bs-shashimoto2048/OCR_Crafter/issues/40)（Model Catalog）**Completed**。新規`src/app/services/model_catalog.py`へ`ModelCatalog`（`list()`/`find()`/`load()`/`exists()`）を実装した。Directory探索は`ModelCatalog`のみが行い、`MetadataReader`（無変更）へは常に単一のPathのみを渡す。同一ベースファイルにCanonical+Legacyが両方存在する場合はCanonical優先・Legacy無視（マージしない）、model_idの重複は走査順で先勝ちのdeduplication。Legacyのmodel_idは暫定的にファイル名を採用（`data/model_ids.json`統合は将来のIssue、[MODEL_CATALOG_DESIGN_NOTES.md](../workitems/model-metadata/MODEL_CATALOG_DESIGN_NOTES.md)参照）。Reader/Adapter由来の例外は握りつぶさず伝播（`ModelCatalogError`はディレクトリ探索エラーのみ）。`inference_model.json`・`.pt`は`list()`の対象外。PR [#41](https://github.com/bs-shashimoto2048/OCR_Crafter/pull/41)をSquash Merge・mainへ反映済み（Merge Commit: `627b6f2`）。**Migration Phase 3（Model Catalog）が完了**。

## 追記7（2026-07-31）: Feature #42でTraining Metadata Factory実装完了（Migration Phase 4: Training Metadata Factory — Completed）

Feature [#42](https://github.com/bs-shashimoto2048/OCR_Crafter/issues/42)（Training Metadata Factory）**Completed**。新規`src/app/services/training_metadata_factory.py`へ、Architecture 6.11で決定済みの単一`ModelMetadataFactory`（Engine別クラス分割なし）を実装した。`create_from_training()`はTraining ResultからModelMetadataを構築して返すのみで、保存（`MetadataWriter`呼び出し・JSON書込）・Directory探索（`MetadataReader`/`ModelCatalog`利用）は一切行わない。Validationは`ModelMetadata.from_dict()`へ完全委譲する。`model_name`→`display_name`・`engine`→`engine_id`へ改名し、対応フィールドの無い`engine_version`・`task`は`extra`へ格納する（フィールド対応の詳細は[TRAINING_METADATA_FACTORY_DESIGN_NOTES.md](../workitems/model-metadata/TRAINING_METADATA_FACTORY_DESIGN_NOTES.md)参照）。呼び出し側`extra`とFactory生成`extra`のキー衝突のみを新設`TrainingMetadataFactoryError`として検出する。PR [#43](https://github.com/bs-shashimoto2048/OCR_Crafter/pull/43)をSquash Merge・mainへ反映済み（Merge Commit: `fee1885`）。**Migration Phase 4（Training Metadata Factory）が完了**。次のOpen項目はModels API連携（[docs/workitems/model-metadata/ISSUE_MAP.md](../workitems/model-metadata/ISSUE_MAP.md)参照）。

## 追記8（2026-07-31）: Feature #44でModels API実装完了（Migration Phase 5: Models API — Completed）

Feature [#44](https://github.com/bs-shashimoto2048/OCR_Crafter/issues/44)（Models API）**Completed**。新規`src/app/services/models_api.py`へ、Architecture 6.17で決定した単一`ModelsAPI` Facade（`list_models()`/`get_model()`/`exists()`/`create_metadata()`/`save_metadata()`）を実装した。`UI → Models API → Catalog / Factory / Writer`の橋渡しのみを行い、Models API自身はMetadataを保持しない。Catalog/Factory/Writerはいずれも無変更、Readerは直接利用しない（読込は常にCatalog経由）。Validationは自前で持たず、Catalog/Factory/Writerへ完全に委譲する。既存`/models/info`（`model_registry.py::list_model_infos()`）への配線は本Featureでは行わず、既存エンドポイントを無変更のまま維持することで後方互換性を保った（調査結果は[MODELS_API_DESIGN_NOTES.md](../workitems/model-metadata/MODELS_API_DESIGN_NOTES.md)参照）。新設`ModelsAPIError`はFacade自体の呼び出し形状不正のみを対象とし、下位層の例外はラップせず伝播させる。PR [#45](https://github.com/bs-shashimoto2048/OCR_Crafter/pull/45)をSquash Merge・mainへ反映済み（Merge Commit: `7fec5fb`）。**Migration Phase 5（Models API）が完了**。

**2026-07-31、Epic #28を一旦停止**: ユーザー判断によりEpic #28配下の新規Feature着手を一旦停止し、UIレビュー（モデル管理・学習・評価画面）を実施する。実際のConsumer切替（`/models/info`のCatalog経由化）・Inference連携等は、UIレビュー完了後に着手要否を判断する。

...
## 追記7（2026-07-31）: Feature #42でTraining Metadata Factory実装完了（Migration Phase 4: Training Metadata Factory — 実装完了・PRレビュー待ち）

Feature [#42](https://github.com/bs-shashimoto2048/OCR_Crafter/issues/42)（Training Metadata Factory）**実装完了・PRレビュー待ち**。新規`src/app/services/training_metadata_factory.py`へ、Architecture 6.11で決定済みの単一`ModelMetadataFactory`（Engine別クラス分割なし）を実装した。`create_from_training()`はTraining ResultからModelMetadataを構築して返すのみで、保存（`MetadataWriter`呼び出し・JSON書込）・Directory探索（`MetadataReader`/`ModelCatalog`利用）は一切行わない。Validationは`ModelMetadata.from_dict()`へ完全委譲する。`model_name`→`display_name`・`engine`→`engine_id`へ改名し、対応フィールドの無い`engine_version`・`task`は`extra`へ格納する（フィールド対応の詳細は[TRAINING_METADATA_FACTORY_DESIGN_NOTES.md](../workitems/model-metadata/TRAINING_METADATA_FACTORY_DESIGN_NOTES.md)参照）。呼び出し側`extra`とFactory生成`extra`のキー衝突のみを新設`TrainingMetadataFactoryError`として検出する。**Migration Phase 4（Training Metadata Factory）が実装完了**。次のOpen項目はModels API連携（[docs/workitems/model-metadata/ISSUE_MAP.md](../workitems/model-metadata/ISSUE_MAP.md)参照）。

...
Related: Epic [#46](https://github.com/bs-shashimoto2048/OCR_Crafter/issues/46)（Engine UI Generalization） / Feature [#47](https://github.com/bs-shashimoto2048/OCR_Crafter/issues/47)（Engine Registry Design、**Completed**・Closed。PR [#48](https://github.com/bs-shashimoto2048/OCR_Crafter/pull/48)をSquash Merge・mainへ反映済み、Merge Commit: `57ca74e`） / Refactor [#57](https://github.com/bs-shashimoto2048/OCR_Crafter/issues/57)（BenchmarkCenterView Engine Label Migration、**Completed**・Closed。PR [#58](https://github.com/bs-shashimoto2048/OCR_Crafter/pull/58)をSquash Merge・mainへ反映済み、Merge Commit: `89b9f9e`） / Design [#59](https://github.com/bs-shashimoto2048/OCR_Crafter/issues/59)（Evaluation UI Generalization Design、**Completed**・Closed。PR [#60](https://github.com/bs-shashimoto2048/OCR_Crafter/pull/60)をSquash Merge・mainへ反映済み、Merge Commit: `ccb172d`。成果物: [EVALUATION_UI_GENERALIZATION.md](EVALUATION_UI_GENERALIZATION.md)） / Epic [#28](https://github.com/bs-shashimoto2048/OCR_Crafter/issues/28)（Unified Model Metadata Infrastructure、完了済み） / Epic [#27](https://github.com/bs-shashimoto2048/OCR_Crafter/issues/27)（TrOCR本体）
...
**状態（2026-08-03）**: Feature #47・Feature #49・Refactor #51（ModelsView Migration）・Refactor [#53](https://github.com/bs-shashimoto2048/OCR_Crafter/issues/53)「TrainingViewをEngine Registryへ移行」はいずれも**Completed**（PR [#54](https://github.com/bs-shashimoto2048/OCR_Crafter/pull/54)をSquash Merge・mainへ反映済み、Merge Commit: `cb6a8ce`）。`TrainingView.jsx`のOCRタイプ選択肢・Engine表示名・学習可否・デバイス対応可否・エンジン固有設定パネル・ジョブスナップショットをRegistry経由へ移行し、UIの見た目は変更していない（既存レンダリング回帰テスト44件が無修正のまま全件成功）。PR #54マージ前レビューで指摘されたRegistry内部状態の不変性（Major #1）も、`Object.freeze()`＋配列Getterのコピー返却により修正済み。詳細は7章「TrainingView」・11章「Future Work」・[docs/workitems/engine-ui/TRAININGVIEW_MIGRATION.md](workitems/engine-ui/TRAININGVIEW_MIGRATION.md)参照。Refactor [#55](https://github.com/bs-shashimoto2048/OCR_Crafter/issues/55)「BenchmarkView Engine Registry Design」も**Completed**（PR [#56](https://github.com/bs-shashimoto2048/OCR_Crafter/pull/56)をSquash Merge・mainへ反映済み、Merge Commit: `2965df7`。設計のみ、Productionコード変更なし）。`BenchmarkView.jsx`の`selectedEngines`はcanonical Engine IDではなくBackend `ENGINE_CATALOG`由来のBenchmark Variant Key軸、`BenchmarkCenterView.jsx`の`row.engine`はcanonical Engine ID軸であり、構造的に異なる2つの「Engine」概念を扱っていることを確定した設計判断として整理した。RunnerとCenterは責務が異なるため統合しない。`BenchmarkView.jsx`のVariant Key構造はEngine Registryへ移さず、`BenchmarkCenterView.jsx`の表示ラベルのみRegistry移行可能と判断した。この判断に基づき、Refactor [#57](https://github.com/bs-shashimoto2048/OCR_Crafter/issues/57)「BenchmarkCenterView Engine Label Migration」も**Completed**（PR [#58](https://github.com/bs-shashimoto2048/OCR_Crafter/pull/58)をSquash Merge・mainへ反映済み、Merge Commit: `89b9f9e`）。`BenchmarkCenterView.jsx`のEngineフィルタ表示値・一覧テーブルのEngine列・比較テーブルのEngine行を`getEngineLabel()`経由の表示へ置換し、フィルタ内部値・比較値・API値はcanonical Engine IDのまま維持、unknown Engineは既知Engineへフォールバックせず「不明」を表示する（`BenchmarkView.jsx`のVariant Key構造・Backend `ENGINE_CATALOG`・CSS/レイアウトは無変更）。TrOCRは既存結果の表示対応のみで機能追加は行っていない。Benchmark RunnerへのTrOCR対応はBackend `ENGINE_CATALOG`側の対応が前提（Epic #27の責務）。`selectedEngines`初期値の再検討・2画面の役割説明改善はFuture Workのまま。詳細は[docs/BENCHMARK_ENGINE_REGISTRY_DESIGN.md](BENCHMARK_ENGINE_REGISTRY_DESIGN.md)参照。

Design [#59](https://github.com/bs-shashimoto2048/OCR_Crafter/issues/59)「Evaluation UI Generalization Design」も**Completed**（PR [#60](https://github.com/bs-shashimoto2048/OCR_Crafter/pull/60)をSquash Merge・mainへ反映済み、Merge Commit: `ccb172d`。設計のみ、Productionコード変更なし）。`OcrEvaluationView.jsx`/`BenchmarkView.jsx`/`BenchmarkCenterView.jsx`の3画面の責務・Engine対応状況・依存API・Registry移行可能性を調査し、以下を確定した設計判断とした。

- `OcrEvaluationView.jsx`はTesseract専用のモデル単体評価画面（Engine軸自体が存在しない、最も限定的なケース）
- `BenchmarkView.jsx`はBenchmark Variant Key軸の実行画面、`BenchmarkCenterView.jsx`はcanonical Engine ID軸の結果閲覧画面（既存の設計判断どおり）
- **3画面は責務・データ軸が異なるため、無理に統合しない**（各画面の責務は維持したまま個別に一般化する）
- `OcrEvaluationView.jsx`の一般化には、Backend評価API（`POST /api/ocr/evaluate`）のマルチEngine対応が前提として必要（whitelist/PSMはTesseract固有機能でRegistryのデータでは代替不可）
- TrOCR Evaluation UIは、Epic [#27](https://github.com/bs-shashimoto2048/OCR_Crafter/issues/27)配下のBackend Evaluation API対応完了後に実施する
- **Epic間の責務境界**: Evaluation UIの画面責務・データ駆動化はEpic #46、Evaluation Backend（Dispatcher・TrOCR対応）はEpic #27の責務。本Epic（#46）の「Evaluation UI Implementation」は、Epic #27配下のMulti-engine Evaluation API設計・実装完了後まで依存待ち（⏸）とする

詳細は[docs/EVALUATION_UI_GENERALIZATION.md](EVALUATION_UI_GENERALIZATION.md)参照。次のDesign Issueは、Epic #27配下の「[Design] Multi-engine Evaluation API Architecture」（Backend評価APIのマルチEngine対応設計）。
...
**状態（2026-08-03）**: **Completed**。Refactor [#55](https://github.com/bs-shashimoto2048/OCR_Crafter/issues/55)「BenchmarkView Engine Registry Design」により詳細調査・設計を実施し、PR [#56](https://github.com/bs-shashimoto2048/OCR_Crafter/pull/56)をSquash Merge・mainへ反映済み（Merge Commit: `2965df7`、設計のみ・Productionコード変更なし）。この設計に基づき、Refactor [#57](https://github.com/bs-shashimoto2048/OCR_Crafter/issues/57)「BenchmarkCenterView Engine Label Migration」で実装まで**Completed**（PR [#58](https://github.com/bs-shashimoto2048/OCR_Crafter/pull/58)をSquash Merge・mainへ反映済み、Merge Commit: `89b9f9e`）。詳細は[BENCHMARK_ENGINE_REGISTRY_DESIGN.md](BENCHMARK_ENGINE_REGISTRY_DESIGN.md)参照。以下は本章時点の要約（詳細調査・実装により裏付け・精緻化済み）。
...
- **BenchmarkView.jsx — できない（Registry対象外）**: `selectedEngines`（`tesseract_model`/`tesseract_base`/`paddleocr_official`/`paddleocr_custom`）は、**エンジンIDとは異なる軸**（Backend `ENGINE_CATALOG`が定義する「モデル取得元＝自作/公式/ベースライン」のvariant key）であり、Registryが扱うエンジン単体の静的情報とは別概念であることを確認した。Registryへ第2軸を持たせるのではなく、Backend `ENGINE_CATALOG`を正としてBenchmark固有概念のまま維持するのが妥当と判断した。本Refactor #57でもVariant Key構造・Backend `ENGINE_CATALOG`は無変更のまま。
- **BenchmarkCenterView.jsx — Registry化済み（Completed）**: 現状生のengine文字列をそのまま表示していた箇所（フィルタの`<option>`テキスト、一覧テーブルのEngine列、比較テーブルのEngine行）を、Registry `getEngineLabel()`経由の表示（`engineDisplayText()`ヘルパー）へ置換した（**完全移行可能**、他画面と異なりRegistryの軸と完全一致）。フィルタ内部値・比較値・API値はcanonical Engine IDのまま維持し、unknown Engineは既知Engineへフォールバックせず「不明」を表示する。CSS・レイアウトの変更なし。
- **BenchmarkCenterView.jsx — できない**: 特になし。本画面は既にエンジン集合をデータから動的導出しており、Registry導入は「表示の質」を上げるのみで構造変更を伴わない。
- **TrOCR対応**: `BenchmarkCenterView.jsx`は既存結果のTrOCR表示（表示ラベルのみ）に対応済み。両画面ともフロントエンド側の障害はほぼ無く、TrOCR追加（Benchmark実行自体への対応）の障害はBackend（`ENGINE_CATALOG`へのvariant key追加・Benchmark実行経路の実装）に集中していることを確認した（Epic #27の責務）。
...
### Refactor #57「BenchmarkCenterView Engine Label Migration」で残った事項

PR [#58](https://github.com/bs-shashimoto2048/OCR_Crafter/pull/58)（Squash Merge・mainへ反映済み、Merge Commit: `89b9f9e`）は9章の優先度2「`BenchmarkCenterView.jsx`のラベル表示置換」に対応する、Scopeを表示ラベルのみへ限定した最小Refactorである。以下は今回対応していない事項。

- **ラベルテーブル統合（9章の優先順位1）は依然未着手のまま**（`ResultBadge.jsx`・`ocrCandidates.js`・`PreprocessView.jsx`・`RapidOCRView.jsx`の独自ラベルテーブルは対象外）。
- **`BenchmarkView.jsx`のVariant Key構造は無変更**。`selectedEngines`はBackend `ENGINE_CATALOG`由来のVariant Key軸のままで、Registry化の対象外（7章・8章の設計判断どおり）。
- **Backend `ENGINE_CATALOG`・API・データ構造は無変更**。本Refactorはフロントエンドの表示テキストのみを変更している。
- **`OcrEvaluationView.jsx`/`BenchmarkView.jsx`/`BenchmarkCenterView.jsx`の3画面統合・Evaluation UI一般化は未着手**。次のFeatureとして「Evaluation UI Generalization」の調査・設計Issueを別途作成する予定（3画面はEngine軸の構造が異なるため、単純なRegistry置換では済まない）。

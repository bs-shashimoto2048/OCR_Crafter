...
- **モデル比較（CER中心・最大3件）**: 上部に🏆推奨モデル（勝利数＋理由=最良を取った指標一覧）→ 比較テーブル（CER/文字正解率/完全一致率/正解/総数/CER改善率/改善/悪化/完全一致へ改善/から悪化＋評価条件行。**各行の最良値を緑ハイライト**。CER・悪化系=最小が最良）→ 混同比較（モデル別件数・最小を緑）→ **勝敗表**（指標ごとの勝者🟢とモデル別勝利数。タイ・欠損は勝者なし）→ モデル基本情報

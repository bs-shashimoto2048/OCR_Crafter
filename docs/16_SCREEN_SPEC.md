...
- **学習前処理タブ**（`lib/preprocessCompare.js buildEffectiveTrainingPreprocess`）: 上部に**「現在の前処理設定」**（v1.0.0で追加。読み取り専用・編集不可）、区切り線を挟んで下部に**「今回学習で使用した前処理」**（学習履歴。既存機能）の2段構成。両方とも**既存の「前処理設定」画面を複製しない読み取り専用の確認表示**（新たな設定値は持たない）。
  ...
  - **今回学習で使用した前処理**（学習履歴・既存機能）: プロジェクトの現在の前処理スナップショット（`GET /api/ocr/training-preprocess/current`）から、**実際に学習パイプラインで使用される工程のみ**（グレースケール/照明ムラ補正/ガンマ補正/CLAHE/局所コントラスト/ヒストグラム平坦化/バイラテラルノイズ除去/シャープ化/アンシャープマスク/二値化方式/オープンクローズ処理/掠れ補正/余白トリミング/傾き補正/リサイズ/ノイズ除去/入力整形）を、**表示名・専門パラメータ名（ホバーで内部パラメータ名を明示）・現在値・単位・説明**の形式で列挙し、無効な工程は薄く表示する。記録が無い場合は`lib/ocrDatasetStatus.js buildTrainingPreprocessStatus`の3状態（旧仕様は2状態。詳細は次項）に応じて案内文を出し分ける（推測補完はしない）
...
- **OCR Dataset作成フローの自動前処理化**（v1.0.0で設計変更。「新規学習データを作成」「再学習データを作成」「データを再作成」の3ボタンが対象。従来のfrom_logs＝ログ由来データセット作成は対象外）: これらのボタンをクリックすると、内部で**必ず`POST /preprocess/run`（現在の前処理設定・overrides省略）→`POST /api/ocr/dataset/create`の順に自動実行**する（`App.jsx createOcrDataset()`）。新しい「前処理を実行」ボタンは追加していない（既存のボタン1つの内部で自動実行）。ボタンは実行中`disabled`になり、ラベルが「1/2 前処理中」→「2/2 Dataset作成中」（スピナー付き）へ切り替わる（`lib/ocrDatasetStatus.js buildDatasetCreateProgressLabel`）。これにより学習データセットは常に現在のprocessed/前処理設定を反映した状態で作成され、学習前処理は必ず「設定記録あり」になる（旧プロジェクトも次回データセット作成で履歴付きへ移行する）

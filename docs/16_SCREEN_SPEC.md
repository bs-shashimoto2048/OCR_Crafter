...
**表示内容**
- 左: 共通設定＋エンジン固有設定（**preparing/training/stopping中はfieldsetで編集ロック**）＋「実行状態」カード（状態・進捗 current/max・進捗バー・経過時間・推定残り時間・Job ID・開始日時）
- 右「学習状況」: 状態サマリー（日本語状態・iteration・経過/残り時間・最新BCER・最新checkpoint・進捗バー）→ 重要イベント一覧（iteration到達/評価/checkpoint保存/展開/完了等のみ抽出）→ ▶詳細ログ（初期は閉。ターミナル形式=等幅1行1ログ・横スクロール・エラー赤/警告黄・自動スクロールON/OFF・最新行へ・フィルタ すべて/重要のみ/警告・エラー）

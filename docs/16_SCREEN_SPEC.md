...
| Step5 | **評価用データ作成**: 上部に「評価画像の取得方法」ラジオ（Step4で作成した画像=従来動作 / フォルダから読み込む=追加機能。フォルダ指定時は評価画像フォルダ入力＋[参照]＋[画像を読み込む]＋画像数＋対応形式 PNG/JPG/JPEG/BMP/TIFF/WEBP の注記を表示。フォルダ直下のみ対象でサブフォルダは対象外。取得方法・フォルダパスはediting_stateへ保存されリロード後に自動復元）。本体は既存ラベル編集と同構成の3カラム（左=候補一覧（仮想スクロール・1000件対応・Series/未入力フィルタ）/ 中央=3画像（元=ユーザー回転適用後の評価画像・中間・最終）＋現在のラベル入力（幅追従38px・配置切替=Step5専用キー `ocr_eval_label_text_align_by_project_v1`）＋OCR候補最大3スロット（Engine/Model/Confidence/小文字ON-OFF/重複スキップ/行単位エラー）＋辞書候補（既存設定参照・Alt+1〜3採用）＋OCR再実行＋保存・保存して次へ・前へ・次へ＋ソフトキーボード / 右=評価画像情報・90°/180°回転・データセット名・正解CSV作成）。OCR候補は `/api/ocr/preview-file` 経由で**回転適用後**の評価画像＋現在プロジェクトのOCR前処理設定を使用（YOLO検出前処理とは無関係）。回転・画像切替はデバウンスして自動再取得（古いレスポンスは破棄・ラベル値は保持）。ショートカット（Enter/Ctrl+S/Ctrl+←→/Esc/Alt+1〜3）は共通hookで既存ラベル編集と同一。前後移動・保存して次へは「表示中（フィルタ適用後）の評価対象チェック済み一覧」基準でSeries外へ移動しない。保存先は評価用editing_state（master.csvへは書かない）。回転は評価用コピーのみ焼き込み・未入力があるとCSV作成不可・リロード後に再開可能・下部ステータス（tabular-nums）。作成完了後は[モデル評価へ]ボタンでモデル評価画面へ遷移し、作成したデータセットを自動選択 |

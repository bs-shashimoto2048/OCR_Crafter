...
| Step5 | **評価用データ作成**（Phase1）: Step4出力画像を候補一覧（仮想スクロール・1000件対応）に表示し、評価対象選択・90°/180°回転（内部0〜270保持・評価用コピーのみ焼き込み）・正解ラベル入力（case-sensitive・Enter=保存して次へ。未入力のみフィルタ時も既存の`decideNextImageIndex`で1件ずつ進む）を行い、`evaluation/<dataset_id>/`（images+ground_truth.csv+metadata.json）を作成。未入力があるとCSV作成不可（「未入力のみ表示」導線あり）。途中状態はプロジェクト配下 `evaluation/editing_state.json` へ自動保存されリロード後に再開可能。下部ステータス（評価対象/登録済み/未入力/回転済み・tabular-nums）。OCR候補・辞書候補等のラベル編集共通化はPhase2 |

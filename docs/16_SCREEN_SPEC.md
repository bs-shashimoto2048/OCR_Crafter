...
- **次回学習の設定はカテゴリサマリー＋インライン編集方式**（モーダル・オーバーレイ・背景暗転は使用しない。学習画面の主要操作のため画面の文脈を遮断しない）: 通常時の本文は「ここで変更した設定は、次回の学習から適用されます。完了済みの学習結果には影響しません。」の明示文言＋**3カテゴリ**のサマリーカード（**学習設定**=エンジン/Iteration or Epoch・演算デバイス・Train/Val/Test比率・Split Seed（学習パラメータとデータ分割の主要値を1枚のカードへ集約）・**オーグメンテーション**=適用項目数/生成倍率/平均適用確率（なし時は「なし」）・**エンジン設定**=Tesseract: eng.traineddata/PSM 7・PaddleOCR: 初期化/Batch）。**カード全体がbutton**（Tab/Enter/Space・aria-label・hover強調・右端「＞」）で、選択すると**左パネルそのものが設定編集表示へ切り替わる**
...
- 編集パネル構成: ←（サマリーへ戻る）＋タイトル＋明示文言のヘッダー → **3タブ**（[学習設定][オーグメンテーション][エンジン設定]。role=tablist/tab/tabpanel・aria-selected・aria-controls。切替しても編集内容を失わない） → 設定フォーム（**この部分のみ縦スクロール**） → 下部操作（**[変更を破棄] [次回学習に適用]**）。ヘッダー・タブ・下部操作はsticky（fitViewport時はflex固定）。最後に開いたタブをlocalStorage `ocr_training_settings_tab_v1` へ保存し初期表示を維持（**旧4タブ構成時代の値`data-split`/`training-params`は`training-settings`へ安全にマッピングし、想定外の値も`training-settings`へフォールバックする**=既存ユーザーがタブを開けなくならない。`lib/trainingSettingsTabs.js`）。**タブは既存の設定値state・保存処理を共有**（View ID・ルート不変・表示の再配置のみ）。preparing/training/stopping中はfieldsetで編集ロック（閲覧は可能）
...
- **オーグメンテーションタブ**（`components/AugmentationSettingsPanel.jsx`＋`lib/augmentationSettings.js`。設定キー・API payload・保存形式・生成倍率の計算は既存のまま）:
  ...
  - 項目行: [チェック] 項目名＋InfoTooltip＋説明 / 確率 (%) / 範囲 ±(°)・±(%) または 強度（弱・中）——単位を必ず明示。xl以上は横1行・狭い画面は2段。**有効行=左端accentバー＋青系背景**、無効行=コントラストを下げ入力disabled（項目名・説明は読める状態を維持）。**無効化しても値は保持され、再ONで直前の値へ復元**（enabledフラグのみ変更）。値は既存範囲（回転0-10°・明るさ/コントラスト0-50%・確率0-100%）へクランプ

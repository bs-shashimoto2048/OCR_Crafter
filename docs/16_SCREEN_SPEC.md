...
- **一覧の列定義（共有定数 `MODEL_LIST_GRID_COLUMNS`）**: ヘッダーと各データ行が同じCSS Grid列定義を共有 `32px minmax(300px,420px) 80px 85px 140px 140px 70px`（選択/モデル名/Engine/方式/作成日/評価/状態）。チェックボックスは視認性・クリックしやすさのため**20px**、モデル名は**15px**（管理Noチップ11px）。**モデル名列は最大420pxの上限付き**で余った幅いっぱいまで伸ばさず、モデル名〜Engine間に大きな空白を作らない。長いモデル名はtruncate＋title（ホバー全文）。最小合計847px未満の幅では一覧コンテナ内のみ横スクロール

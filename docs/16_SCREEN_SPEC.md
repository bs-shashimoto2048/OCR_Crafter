...
- **学習前処理タブ**（v1.0.0で追加。`lib/preprocessCompare.js buildEffectiveTrainingPreprocess`）: **既存の「前処理設定」画面を複製しない読み取り専用の確認表示**（新たな設定値は持たない。学習前処理は常に「前処理設定」画面の値がそのまま使用される）。プロジェクトの現在の前処理スナップショット（`GET /api/ocr/training-preprocess/current`）から、**実際に学習パイプラインで使用される工程のみ**（グレースケール/照明ムラ補正/ガンマ補正/CLAHE/局所コントラスト/ヒストグラム平坦化/バイラテラルノイズ除去/シャープ化/アンシャープマスク/二値化方式/オープンクローズ処理/掠れ補正/余白トリミング/傾き補正/リサイズ/ノイズ除去/入力整形）を、**表示名・専門パラメータ名（ホバーで内部パラメータ名を明示）・現在値・単位・説明**の形式で列挙し、無効な工程は薄く表示する。記録が無い場合は`lib/ocrDatasetStatus.js buildTrainingPreprocessStatus`の3状態（**v1.0.0で2状態から変更**。詳細は次項）に応じて案内文を出し分ける（推測補完はしない）

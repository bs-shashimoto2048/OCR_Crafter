...
- **次回学習の設定はカテゴリサマリー＋インライン編集方式**（モーダル・オーバーレイ・背景暗転は使用しない。学習画面の主要操作のため画面の文脈を遮断しない）: 通常時の本文は「ここで変更した設定は、次回の学習から適用されます。完了済みの学習結果には影響しません。」の明示文言＋4カテゴリのサマリーカード（**データ分割**=比率/Split Seed・**オーグメンテーション**=適用項目数/生成倍率/平均適用確率（なし時は「なし」）・**学習パラメータ**=エンジン/Iteration or Epoch/演算デバイス・**エンジン設定**=Tesseract: eng.traineddata/PSM 7・PaddleOCR: 初期化/Batch）。**カード全体がbutton**（Tab/Enter/Space・aria-label・hover強調・右端「＞」）で、選択すると**左パネルそのものが設定編集表示へ切り替わる**
...
- 編集パネル構成: ←（サマリーへ戻る）＋タイトル＋明示文言のヘッダー → **4タブ**（[データ分割][オーグメンテーション][学習パラメータ][エンジン設定]。role=tablist/tab/tabpanel・aria-selected・aria-controls。切替しても編集内容を失わない） → 設定フォーム（**この部分のみ縦スクロール**） → 下部操作（**[変更を破棄] [次回学習に適用]**）。ヘッダー・タブ・下部操作はsticky（fitViewport時はflex固定）。最後に開いたタブをlocalStorage `ocr_training_settings_tab_v1` へ保存し初期表示を維持。**タブは既存の設定値state・保存処理を共有**（View ID・ルート不変・表示の再配置のみ）。preparing/training/stopping中はfieldsetで編集ロック（閲覧は可能）
...
- タブ内訳: **データ分割**=プロジェクト/学習データ（読み取り）/学習データ作成方法（新規・再学習）＋再学習時オプション/分割比率/Split Seed/[分割枚数を確認] ・ **学習パラメータ**=OCRタイプ/最大イテレーション（Tesseract）or 学習回数（PaddleOCR）/演算デバイス/出力先 ・ **エンジン設定**=従来のエンジン固有設定（Tesseract: Base Model/PSM/Charset/Whitelist/実験情報、PaddleOCR: 初期重み/実行プロファイル/ワーカー/AMP/charset/画像形状/バッチ/最大文字数）

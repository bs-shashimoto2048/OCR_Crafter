...
- **レイアウト（流体2カラム）**: 1250px以上=左右2カラム `minmax(0,1.8fr) : minmax(0,1fr)`（≈64:36。**両列とも minmax(0,…) で固定最低幅を持たず**、ブラウザ幅に応じて連続的に収縮＝右ペインがはみ出さない・ページ全体に横スクロールを出さない）/ 1250px未満=右ペインを下段へ縦積み（固定高を外し自然高。一覧は max-h-60vh で内部スクロール。文字を縮小して押し込まない）。一覧の列幅: モデル名=min 280pxの可変・Engine/方式=90px・作成日/評価=150px・状態=80px（一覧が収まらない幅では一覧コンテナ内のみ横スクロール）

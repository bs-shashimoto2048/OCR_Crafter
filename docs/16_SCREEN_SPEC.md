...
| Step2 | YOLO検出（モデル選択・信頼度しきい値・重複マージ）。モデル一覧は「プロジェクト内 `models/yolo` → 共通 `models/yolo`（リポジトリ直下）→ ビルトイン」の順（同名はプロジェクト内優先）。保存済み選択が一覧に無い場合は黙って別モデルへ置き換えず「（見つかりません）」表示＋警告を出す。検出0件は「検出結果: 0件（正常終了）」、処理失敗は「YOLO検出に失敗しました。理由: …」で区別表示。画像・プロジェクト切替後に届いた古い検出レスポンスは破棄（リクエスト連番ガード） |

...
- **学習前処理タブ**（v1.0.0で追加。`lib/preprocessCompare.js buildEffectiveTrainingPreprocess`）: **既存の「前処理設定」画面を複製しない読み取り専用の確認表示**（新たな設定値は持たない。学習前処理は常に「前処理設定」画面の値がそのまま使用される）。プロジェクトの現在の前処理スナップショット（`GET /api/ocr/training-preprocess/current`。一度も前処理を実行していないプロジェクトは`training_preprocess=null`）から、**実際に学習パイプラインで使用される工程のみ**（グレースケール/照明ムラ補正/ガンマ補正/CLAHE/局所コントラスト/ヒストグラム平坦化/バイラテラルノイズ除去/シャープ化/アンシャープマスク/二値化方式/オープンクローズ処理/掠れ補正/余白トリミング/傾き補正/リサイズ/ノイズ除去/入力整形）を、**表示名・専門パラメータ名（ホバーで内部パラメータ名を明示）・現在値・単位・説明**の形式で列挙し、無効な工程は薄く表示する。未記録（前処理未実行）は「未記録。このプロジェクトではまだ「前処理設定」画面で前処理が実行されていません」と表示し、現在のプロジェクト設定から推測して埋めない
...
- **学習前処理の実行状況カード**（v1.0.0で追加。`lib/ocrDatasetStatus.js buildTrainingPreprocessStatus`）: 学習データサマリーの直下に「学習前処理」カードを表示し、`GET /api/ocr/training-preprocess/current`の`executed`/`executed_at`/`processed_image_count`から「✔ 実行済み（処理画像数: n / 処理日時: YYYY-MM-DD HH:MM:SS）」または「○ 未実行」を表示する。「前処理は終わっているか」を学習データを確認する前に一目で判断できるようにするためのもの（推論・評価とは独立の表示専用情報で、新たな設定値は持たない）

...
- 中央: 元画像・中間画像・最終画像（3段プレビュー）、OCR候補（最大3モデル・差分黄色表示・常時3行）、辞書からの近似候補（差分は蛍光緑 `#adff5d` で強調表示）、ラベル入力（画像表示エリアと同幅・32px・min-height 64px・見出し右の「文字位置」で左/中央/右を切替＝プロジェクト別保存）、ソフトキーボード

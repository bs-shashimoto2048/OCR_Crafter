...
- **次回学習の設定はカテゴリサマリー＋設定モーダル方式**（狭い左カラムで全設定を直接編集しない）: 本文は「ここで変更した設定は、次回の学習から適用されます。完了済みの学習結果には影響しません。」の明示文言＋4カテゴリのサマリー行（**データ分割**=比率/Split Seed・**オーグメンテーション**=適用項目数/生成倍率（なし時は「なし」）・**学習パラメータ**=エンジン/Iteration or Epoch/演算デバイス・**エンジン設定**=Tesseract: eng.traineddata/PSM 7・PaddleOCR: 初期化/Batch）＋各行の[編集]ボタン。[編集]で**設定モーダル**（document.bodyへポータル描画・max-w-6xl・Esc/×/背景クリックで閉じる・role=dialog）を開く
- 設定モーダルは**4タブ切替**（[データ分割][オーグメンテーション][学習パラメータ][エンジン設定]。role=tablist/tab/tabpanel・aria-selected・aria-controls）。最後に開いたタブをlocalStorage `ocr_training_settings_tab_v1` へ保存し初期表示を維持。**タブは既存の設定値state・保存処理を共有**（View ID・ルート不変・表示の再配置のみ）。preparing/training/stopping中はモーダル内fieldsetで編集ロック（閲覧は可能）
...
- **オーグメンテーションタブ**（`components/AugmentationSettingsPanel.jsx`＋`lib/augmentationSettings.js`。設定キー・API payload・保存形式・生成倍率の計算は既存のまま）:
  ...
  - 設定一覧(70%)＋プレビュー(30%)の2カラム（lg以上。プレビューはsticky。横幅不足時は1カラムへ）。項目は3カテゴリへ分類: **幾何変換**（回転）/**明るさ・コントラスト**（明るさ・コントラスト）/**ノイズ・ぼかし**（ぼかし（ガウシアン）・ノイズ）——既存の設定キー5項目のみで存在しない変換は追加しない（除外設定に相当する機能は未実装のため表示しない）

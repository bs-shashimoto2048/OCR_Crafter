...
- **レイアウト**: 1400px以上=左右2カラム（左=一覧 `minmax(0,2fr)` / 右=詳細・比較 `minmax(520px,1fr)`。右ペインは最低520px） / 1400px未満=右ペインを下段へ縦積み（固定高を外し自然高。一覧は max-h-60vh で内部スクロール。文字を縮小して押し込まない）。一覧の列幅: モデル名=min 280pxの可変・Engine/方式=90px・作成日/評価=150px・状態=80px

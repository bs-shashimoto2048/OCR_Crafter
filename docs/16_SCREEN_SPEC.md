...
- **モデル比較（比較ダッシュボード・CER中心・最大3件・管理No主体）**: パネルタイトル「モデル比較 選択中 n件 / 最大3件」。**固定領域**（凡例 → 警告 → 推奨モデル → 主要3指標カード。画面が低い場合のみ内部スクロールし、下の詳細分析の最低高150pxを確保）＋**スクロール領域**（改善・悪化比較 → 評価条件 → 混同比較 → 指標別結果 → 総合勝利数 → モデル詳細情報）の2層構成で、主要指標を確認したまま詳細分析をスクロールできる
  ...
  - **混同比較**: 全モデル合計の多い順（比較中は同じ混同が同じ位置）。**モデル別の横棒グラフ**（件数併記・0件は棒なし・旧形式=混同データなしは—）。初期TOP8・「すべて表示」で全件展開。挿入/脱落は `∅→1` / `Y→∅` 表記

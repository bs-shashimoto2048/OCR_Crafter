...
- **推奨バッジ6種**（🏆Best CER=最新評価でCER最小 / 🏆Best Char Acc=文字正解率最大 / 🏆Best Accuracy=完全一致率最大 / 🟢Recommended=総合推奨（CER→文字正解率→Accuracy→悪化件数の優先順。Accuracy単独では決めない）/ ⭐Latest Best=最新の評価実行で最良 / 🔵Baseline=eng系ベースモデル。評価履歴から自動判定・手動設定なし）を一覧・詳細・比較に表示
- 最新モデル表示（PaddleOCR / Tesseract 並記）
- **モデル詳細（モデルカルテ）**: モデル情報（Engine・方式・ベースモデル・Charset・Iteration・**学習画像数・モデルサイズ**・学習時間・学習日時・Export状態）→ **最新評価**（**CERの大表示**・文字正解率・完全一致率=業務指標（正解/総数＋%）・誤認識件数・CER改善（pt差＋相対）・改善/同等/悪化・完全一致の増減）→ **混同TOP5** → **評価条件**（評価データセット・画像数・OCR前処理・Whitelist・評価日時）→ **評価履歴**（日時/CER/文字/一致/正解/改善/悪化/前処理の表。**残り高を使って内部スクロール・ヘッダーsticky固定**）。上部情報は不足時のみ内部スクロール。旧形式の履歴（詳細情報なし）は「未記録」表示
- **モデル比較（CER中心・最大3件・管理No主体）**: 列ヘッダー・推奨モデル・勝敗表勝者（🟢 M0004）・勝利数は**管理No表示**（ファイル名はツールチップと推奨カード/モデルカードの下部に補助表示。長いファイル名で列幅が潰れない）。上部に🏆推奨モデル（M番号主体＋勝利数＋理由=最良を取った指標一覧）→ 比較テーブル（CER/文字正解率/完全一致率/正解/総数/CER改善率/改善/悪化/完全一致へ改善/から悪化＋評価条件行。**各行の最良値を緑ハイライト**。CER・悪化系=最小が最良）→ 混同比較（モデル別件数・最小を緑）→ **勝敗表**（指標ごとの勝者🟢とモデル別勝利数。タイ・欠損は勝者なし）→ モデル基本情報（管理No＋ファイル名）。管理No未付与（旧レスポンス等）は従来の短縮名へフォールバック

...
**表示内容**
- モデル一覧（種別・作成日時・学習条件・Alias・評価情報＝Accuracy・**正解/総数・改善率・評価日時**。一覧だけで性能比較できる）
- **推奨バッジ**（🟢推奨=最新評価で最高性能 / 🏆Best Accuracy=全評価履歴で最高 / 🔴ベースライン=eng系。評価履歴から自動判定・手動設定なし）を一覧・詳細に表示
- 最新モデル表示（PaddleOCR / Tesseract 並記）
- **モデル詳細（モデルカルテ）**: モデル情報（Engine・方式・ベースモデル・Charset・Iteration・**学習画像数・モデルサイズ**・学習時間・学習日時・Export状態）→ **最新評価**（認識率の大表示＋バー・正解/総数・誤認識件数・学習前比の改善率/改善件数）→ **評価条件**（評価データセット・画像数・OCR前処理・Whitelist・評価日時）→ **評価履歴**（日時/セット/Acc/正解/改善率/前処理の表。**残り高を使って内部スクロール・ヘッダーsticky固定**）。上部情報は不足時のみ内部スクロール。旧形式の履歴（詳細情報なし）は「未記録」表示
...
- 認識率・増減・改善率・誤認識一覧（結果カードに使用した評価データセット名・枚数・作成日時と、**評価実行時にサーバーが実際に適用した前処理**（応答のecho。UI選択中の値ではない）を表示。旧形式の結果は「未記録」）
- 評価実行中はボタン下へ状態表示＋アニメーションバー（サーバーが同期処理のため段階別進捗は表示しない）
- 評価履歴テーブル（モデル / 評価データセット / Accuracy / **前処理** / 日時。localStorage履歴から最大50行・内部スクロール。前処理列は短いサマリー＋ツールチップで詳細、旧形式の履歴は「未記録」）

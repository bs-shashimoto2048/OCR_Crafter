...
リクエストスキーマは `src/app/schemas.py` を参照。**全142エンドポイント**（GET 72 / POST 57 / PUT 4 / PATCH 3 / DELETE 6）。
...
| Method / Path | リクエスト | レスポンス主要キー | 概要 |
|---|---|---|---|
| POST `/api/ocr/dataset/create` | `OcrDatasetCreateRequest`（`image_types`, `charset`, `max_text_length`, `image_shape`, 比率, `seed`, `text_case`, 任意: `augmentation`=新形式オーグメンテーション設定） | 作成結果（`counts`＋`input_count`/`valid_count`/`skipped`内訳/`split_method: image`/`augmentation`/`augmentation_generated`） | ラベル済み画像からOCR認識用データセット作成。**v1.0.0で追加**: フロント（`App.jsx createOcrDataset()`）は、このAPIを呼ぶ**前に必ず`GET /api/ocr/preprocess/current-config`で保存済みの学習用前処理設定を確認**し、①未保存（`saved_config: null`）なら作成を開始しない（「学習用の前処理設定が保存されていません。前処理画面で設定内容を確認し、「前処理設定保存」を実行してください。」）、②保存後に変更あり（`is_saved: false`）なら「前処理設定を保存」「保存時の設定に戻す」「キャンセル」の選択を経てからのみ続行、③一致していれば`POST /preprocess/run`へ**保存済みconfigを明示的にoverridesとして渡し**（現在設定を暗黙取得するだけの方式にしない）processed/を最新化してから、このAPIを呼ぶ（学習データセット作成ボタン1つの内部で自動実行。新しい「前処理を実行」ボタンは追加していない。既存の`/preprocess/run`をそのまま再利用し前処理ロジックは複製しない）。これにより学習データセットは必ず「保存済みの学習用前処理設定」から作成される。このAPI自体（`create_ocr_dataset`）は無変更——既存の`processed/`とスナップショットを読み取るだけの従来動作を維持しており、直接このAPIのみを呼ぶ場合は引き続き未処理なら`training_preprocess: null`のまま（後方互換）。**分割枚数は最大剰余法**（合計=有効画像数を保証。同値小数部はTrain→Val→Test優先）。`augmentation` 指定時は**Trainのみ**へ追加画像を生成（元画像は必ず残る・ラベル不変・生成枚数=(倍率-1)×Train枚数）。比率合計≠1.0は **400**（`detail={code: INVALID_SPLIT_RATIO, message, values}` の構造化エラー）。**meta.json へ学習時前処理を確定保存**: `training_preprocess`（作成時点の `processed/meta/preprocess_snapshot.json` のコピー＋`ocr_input_normalization`。スナップショット未保存の旧プロジェクトは `null`=推測補完しない）・`training_preprocess_hash`（sha256）・`source_image_state`（processed/mixed等）・`source_priority`・`source_state_counts`・`source_preprocess_snapshot_id`・`source_warning`（processed以外の混在時、または1件もprocessedが無い＝前処理未実行時に警告文を設定。**v1.0.0で追加**: 従来は混在時のみ警告しており、全件未処理は無警告だった）。**v1.0.0で追加**: `augmentation_hash`（オーグメンテーション設定のsha256。前処理ハッシュとは別概念・未使用時はnull）・`training_input_pipeline_hash`（`training_preprocess_hash`と`augmentation_hash`を結合したsha256。いずれか一方でも記録があれば生成）。**v1.0.0で追加**: `preprocess_config_version`・`preprocess_config_saved_at`（「前処理設定保存」で確定した学習用設定のVersion・保存日時。保存済み設定の`config_hash`が実際に使用した`training_preprocess_hash`と一致する場合のみ記録し、一致しない/保存済み設定が無い場合は`null`のまま=過去設定を推測しない） |
| POST `/api/ocr/dataset/split-preview` | `OcrDatasetSplitPreviewRequest`（`image_types`, `charset`, `max_text_length`, `text_case`, 比率） | `input_count`, `target_count`, `labeled_count`, `valid_count`, `skipped{type,invalid_label,empty_label,charset_invalid,length_exceeded,missing_source}`, `counts`, `split_method`, `ratios`, `source_image_state`, `source_state_counts`, `source_warning` | データセット作成前の分割予定枚数プレビュー（画像は生成しない。作成時と同じ最大剰余法）。**v1.0.0で追加**: `target_count`（選択画像種別に一致する行数）・`labeled_count`（うちラベル入力済みの行数）・`skipped`のラベル不正内訳を`empty_label`/`charset_invalid`/`length_exceeded`へ分離（`invalid_label`は後方互換で3者の合算値を維持）。**v1.0.0で追加**: `source_image_state`/`source_state_counts`/`source_warning`（学習ソース画像の由来。`create_ocr_dataset`と同じ`summarize_source_states`を共用し、作成前に前処理未実行を警告できるようにする） |
| GET `/api/ocr/dataset/latest` | Query: `project_id?` | `{project_id, dataset}`（`dataset`=`null` または `{dataset_id, dataset_root, source, created_at, charset, seed, counts, training_preprocess_hash, augmentation_hash}`） | **v1.0.0で追加**。プロジェクト内で最後に作成された学習データセットをディスク上のmeta.jsonから復元する読み取り専用API（`ocr_dataset`/`ocr_dataset_from_logs`の両方を対象に`created_at`最新を1件返す）。何も作成しない。フロント側の作成状態（React state）はページ再読み込みで失われるため、その復元に使う。`dataset_id`は永続IDではなく、作成順のフォルダ名から導出した表示専用値（`DS-<フォルダ名>`） |
...
| POST `/api/trocr/train/start` | `TrocrTrainStartRequest`（`dataset_dir`, `model_ref`, `epochs`, `batch_size`, `learning_rate`, `max_target_length`, `device`: `auto`/`cpu`/`cuda`, `local_files_only`） | `job_id`, `engine: trocr` | **Epic #27 Feature #94で追加**。TrOCR fine-tune開始（Tesseract/PaddleOCRと同様の専用エンドポイント。既存`/api/ocr/train/start`は汎用化しない）。二重実行は**409 Conflict**。`device=cuda`はCUDA利用不可環境で**400**。Job lifecycle（状態取得・停止・ログ取得）は`training_family="ocr"`のためEngine非依存の既存API（`GET /api/ocr/train/status/{job_id}`等）をそのまま使う。学習成功時は`.trocr.json`sidecarへ自動登録（Feature #96、登録失敗時はJobを`failed`として扱う） |
| GET `/api/trocr/models` | Query: `project_id?` | `{project_id, items[]}`（各item: `.trocr.json`sidecarそのもの＝`name, engine, model_dir, base_model_ref, project_id, job_id, dataset_root, dataset_id, epochs, batch_size, learning_rate, final_loss, created_at`） | **Epic #27 Feature #98で追加**。登録済みTrOCRモデル一覧（Feature #96 `list_trocr_models()`の薄いラッパー）。Training UIの「登録済みモデルから継続Fine-tune」選択に使う。`GET /models`/`GET /models/info`（一般Modelsリスト）へは統合しない（Feature #96のFuture Work境界を維持） |
...
| GET `/api/ocr/preprocess/current-config` | Query: `project_id?` | `{project_id, current_preprocess, current_preprocess_hash, saved_config, is_saved}` | **v1.0.0で追加**。「次回 `/preprocess/run` 実行時に使用される現在の前処理設定」を返す読み取り専用API（プロジェクト保存値`preprocess_config.json`が無ければsettings.yaml既定を使用。`build_preprocess_config`→`build_preprocess_snapshot`→`build_training_preprocess`で、上記`training-preprocess/current`の`training_preprocess`と**同一のstructure**を構築するが、既存の学習履歴（前処理実行時に確定保存されたスナップショット）とは別概念のためフィールド名を`current_preprocess`/`current_preprocess_hash`へ分離している。前処理の実行・processed生成・スナップショット保存は一切行わない（完全に副作用なしの読み取り専用）。学習画面「学習前処理」タブの「現在の前処理設定」欄（編集不可・表示専用）が参照する。**v1.0.0で追加**: `saved_config`（「前処理設定保存」で確定した学習用設定`{version, saved_at, config_hash, training_preprocess, overrides}`。無ければnull）・`is_saved`（現在設定と保存済み設定のHash一致=未保存の変更が無いか。既存の正式なHash生成`compute_training_preprocess_hash`で判定し、React state比較やJSON文字列比較はしない）。`saved_config.overrides`は`training_preprocess_to_config`で変換済みの、そのまま`/preprocess/run`のoverridesへ渡せる形 |
| GET `/api/ocr/preprocess/saved-config` | Query: `project_id?` | `{project_id, saved_config, history[]}` | **v1.0.0で追加**。確定済み学習用前処理設定と保存履歴（バージョン降順）を返す読み取り専用API。プリセット（複数保存・再利用可能なテンプレート）とは別概念——プロジェクトごとに確定設定を1件＋履歴のみ持つ。履歴は初回実装では読み取り専用（過去Versionへの復元機能は無い） |
| POST `/api/ocr/preprocess/saved-config` | `PreprocessSavedConfigRequest`（`project_id?`） | `{project_id, created, saved_config}` | **v1.0.0で追加**。現在の解決済み前処理設定を「学習に使用する確定済み設定」として保存する（`build_preprocess_config`→`build_preprocess_snapshot`→`build_training_preprocess`を再利用。別形式・別Hashロジックは作らない）。同一Hashの再保存は履歴を増やさず既存の確定設定を返す（`created: false`）。**全画像への前処理再実行は行わない**（processed生成はDataset作成時のみ）。保存先は`project_root/preprocess/saved_config.json`＋`project_root/preprocess/history/v{NNNN}.json`（version単調増加・履歴は上書きされない） |
| POST `/api/ocr/preprocess/saved-config/restore` | `PreprocessSavedConfigRequest`（`project_id?`） | `{project_id, overrides, restored_version}` | **v1.0.0で追加**。確定済み設定を現在のプロジェクト前処理設定（次回`/preprocess/run`実行時に使う値=`preprocess_config.json`）へ復元する。`training_preprocess_to_config`で変換した設定を既存の`save_project_preprocess_overrides`へそのまま書き込む（プリセット読込とは異なり実際のプロジェクト設定へ反映する）。**全画像への前処理再実行は行わない**。保存済み設定が無い場合は**404** |
| GET `/api/ocr/inference/model` | Query: `project_id?` | `{project_id, inference_model}` | **v1.0.0で追加**。プロジェクト単位で保存されている推論使用モデルの選択（`{engine, model, inference_model_id, updated_at}`）を返す読み取り専用API。無ければ`inference_model: null`（推測補完しない） |
| POST `/api/ocr/inference/model` | `InferenceModelSaveRequest`（`project_id?`, `engine`, `model`, `model_id?`） | `{project_id, inference_model}` | **v1.0.0で追加**。推論使用モデルの選択をプロジェクト単位で保存する（利用者が選択した時点で即時保存。保存用の別ボタンは無い）。保存先は`project_root/inference_model.json`（既存の`preprocess_config.json`と同じ「project_root直下のフラットJSON」方式を再利用し、新しい保存機構は増やさない） |
...
| GET `/api/ocr/datasets/{dataset_id}` | Query: `project_id?` | 基本情報＋`preprocess{version,saved_at,hash}`＋`training_settings{train_ratio,val_ratio,test_ratio,charset,rotation{enabled,max_degrees},input_count,excluded_count}`＋`counts`＋`models[{name,model_id}]`＋`experiments[{experiment_id,created_at,models[]}]` | Dataset詳細。存在しないIDは**404**。`rotation`はDataset作成時のオーグメンテーション回転設定（`meta.augmentation.rotation`）を指す（画像個別回転APIとは別概念）。**v1.0.0で追加（Experiment Manager強化）**: `experiments`＝このDatasetを使用したExperiment一覧（既存の`experiment_tracker.list_experiments_for_dataset`を再利用。Dataset⇔Experimentも永続的な逆引きインデックスは持たずExperiment側の`dataset_id`で都度フィルタする） |
...
| Method / Path | リクエスト | レスポンス主要キー | 概要 |
|---|---|---|---|
| GET `/api/experiments` | Query: `project_id?` | `{project_id, items:[{experiment_id, created_at, started_at, finished_at, duration_seconds, models[], model_ids[], experiment_name, parent_model_id, note, operator, model_engine, dataset_id, dataset_name, dataset_hash, training{iterations, charset, base_lang, split_ratio, split_seed, split_method, counts, optimizer, scheduler, loss, learning_rate, batch_size}, preprocess{hash, snapshot_id, summary, version}, augmentation{config, generated}, evaluation, tags[], favorite, source}]}` | 実験一覧。実験IDは **EXP-0001形式・プロジェクト内一意・作成順・再利用しない**（モデル管理Noとは独立。1実験に複数モデルを紐付け可能=modelsはリスト）。学習完了時（`register_tesseract_model`）に自動記録され、**実験記録のない旧モデル（.tess.json）は一覧取得時に自動バックフィル**（source="backfill"・作成日時順採番）。保存先は `data/projects/<id>/experiments.json`。**v1.0.0で追加（Experiment Manager強化。既存のExperiment Trackingをそのまま拡張——新しい実験保存形式・別IDは作らない）**: `model_engine`（学習エンジン。Tesseract学習完了時（`tesseract_pipeline.py`）・TrOCR学習完了時（`trocr_model_registry.py`）にそれぞれ`"tesseract"`/`"trocr"`が自動記録される。TrOCRでも正しく記録されることをIssue #164のE2E検証で確認済み。**PaddleOCR学習完了時は`record_experiment()`を呼ばないため自動記録されない**——バックフィル対象にもならず、実験カルテはTesseract/TrOCRの学習でのみ自動生成される）・`dataset_id`/`dataset_name`（Dataset Managerが確定保存した値をそのまま流用）・`dataset_hash`（Dataset自体の内容ハッシュという独立概念が既存に無かったため、既存の`training_input_pipeline_hash`＝前処理+オーグメンテーションの結合ハッシュをそのまま再利用）・`training.optimizer`/`scheduler`/`loss`/`learning_rate`/`batch_size`（Tesseract LSTM fine-tuneには概念が無いため常に`null`。存在しない情報を推測で埋めない）・`preprocess.version`（「前処理設定保存」のVersion番号。Dataset側の`preprocess_config_version`と一致する場合のみ記録） |
| DELETE `/api/experiments/{experiment_id}` | Query: `project_id?` | `{project_id, experiment_id, deleted}` | **v1.0.0で追加**。Experimentカルテのみを削除する（`experiments.json`からエントリを除くだけ。Dataset・Model・評価結果の実体には一切影響しない）。存在しないIDは**404**。監査対象操作（`experiment_delete`・operator以上） |
| PATCH `/api/experiments/{experiment_id}` | `ExperimentUpdateRequest`（`tags?`, `favorite?`, `note?`, `operator?`, `experiment_name?`） | `{item}` | 実験カルテの編集（自由タグ最大20件・★固定・メモ・学習者・実験名のみ。学習条件は不変）。存在しないIDは404。`note`は複数行対応（Markdown不要・プレーンテキストとして保存）——**v1.0.0でUIに複数行編集用のインラインテキストエリアを追加**（既存APIはそのまま） |
...
| Method / Path | リクエスト | レスポンス主要キー | 概要 |
|---|---|---|---|
| GET `/api/benchmarks/engines` | - | `{items:[{key, label, implemented, available, availability_note, description}]}` | 対応エンジンカタログ＋実行環境での利用可否。`implemented: true`= `tesseract_model` / `tesseract_base` / `paddleocr_official` / `paddleocr_custom` / `trocr`。**EasyOCRのみ`implemented: false`＝「未導入・利用不可」明示・実行対象外**（`services/benchmark.py::ENGINE_CATALOG`参照）。クラウドOCRは対象外（掲載しない） |
...
| DELETE `/models/{model_name}` | Query: `project_id?` | `deleted` | モデル削除（models配下限定の安全検証あり）。**v1.0.0で追加**: 削除対象が保存済みの推論使用モデル（`inference_model.json`）と一致する場合、保存済み選択を自動クリアする（削除済みモデルを指したままにしない。フロント側は削除前に「このモデルは現在、推論使用モデルに設定されています。削除すると推論モデルが未選択になります。削除しますか？」を確認表示）。**モデル削除はDataset側のリンクを一切変更しない**（Dataset⇔Modelのリンクは`list_model_infos`とのライブ集計のため、モデルファイルが無くなれば自動的にDatasetの使用モデル数から外れる＝明示的な解除処理は不要）。**Feature #141で追加**: `.trocr.json`（TrOCR、sidecar＋`model_dir`ディレクトリ一式）も同じ安全検証経路で削除できる |
| GET `/api/models/download/{model_name}` | Query: `project_id?` | FileResponse | `.pt` / `.traineddata` / inference ZIP のダウンロード。**Feature #141で追加**: `.trocr.json`（TrOCR）は`model_dir`ディレクトリ一式＋sidecarを`<name>.trocr.zip`としてZIP化（Deployment Package #117の`_add_directory_artifact_to_zip()`を再利用、新形式は追加しない） |

...
| POST `/api/ocr/preview-file/batch` | multipart: preview-fileと同じ入力（`file?` / `export_id`+`filename`+`rotation` / `source_directory`+`filename`+`rotation`, `project_id`, `overrides_json?`, `eval_preprocess_json?`）+ `slots_json`（最大3スロットの配列 `[{slot, engine, model, model_type?, easyocr_langs, include_lowercase, psm, whitelist}]`。`[]`=プレビューのみ） | `{project_id, type, ratio, original_size, pipeline, interim_data_url, processed_data_url, results:[{slot, engine, model_name, prediction, confidence, error, cached}]}` | Step5用: **前処理1回＋複数OCR設定を1リクエストで処理**。画像デコード・回転・Step5専用前処理・共通前処理・base64生成は1回だけ実行し全スロットで共有。スロットは**番号順に逐次実行（同時実行数1）**でCPUエンジンの競合を回避。1件失敗しても他スロットの結果は返す（行単位error）。同一の処理済み画像sha256×同一設定の結果は**プロセス内LRUキャッシュ（128件）を再利用**（エラーは対象外）。中間・最終画像はレスポンス直下に1回だけ含め、スロット結果には含めない。既存 preview-file は後方互換のため維持 |

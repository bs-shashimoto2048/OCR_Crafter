...
| 項目 | 内容 |
|---|---|
| OCR自動実行は既定OFF | 画像切替・回転・前処理変更・OCR設定変更では候補を「要再実行」表示にするだけでOCRは実行しない（性能対策）。自動実行はStep5専用設定でONにできるが、連続操作終了後に1回だけ実行される |
| OCR結果キャッシュ | サーバー側は処理済み画像sha256+推論設定キーのLRU（プロセス内・最大128件・エラーは対象外）。フロント側は実行条件キーのLRU（セッション内・最大30件）。プロセス/ページ再読込で消える |
| スロット実行は逐次 | バッチAPI内でスロット番号順に1件ずつ実行（同時実行数1）。CPUエンジン同士の競合を避ける設計で、3スロット合計時間は各エンジンの合算になる |
| バッチAPIはリクエスト単位で中断不可 | フロントはAbortControllerで通信を中止するが、サーバー側で開始済みのOCR推論自体は完了まで走る（同時実行1のため影響は限定的） |

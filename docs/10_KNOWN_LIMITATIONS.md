...
| スロット実行は同時2並列 | バッチAPI内でスロット番号順にsubmitし同時実行数2で並列実行（3スロット時は先頭2件が先行）。3スロットの合計時間は最長エンジン＋αになる。同時に多数のバッチ要求が来た場合はCPU競合で遅くなり得る |
| バッチAPIはリクエスト単位で中断不可 | フロントはAbortControllerで通信を中止するが、サーバー側で開始済みのOCR推論自体は完了まで走る（古い結果はフロントで破棄される） |
| 先読みとの競合 | 次画像の先読み中にその画像へ移動すると、先読みと通常実行が同一条件で二重に走る場合がある（結果は同一で表示は正しい。サーバー結果キャッシュにより片方は再計算なしで済むことが多い） |

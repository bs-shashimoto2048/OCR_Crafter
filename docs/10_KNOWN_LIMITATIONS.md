...
| モデル評価は同期API・段階進捗なし | `/api/ocr/evaluate` は完了まで1リクエストで実行される（実行中は状態表示＋アニメーションバーのみで「前処理中 n/N」等の段階別進捗は表示しない）。大量画像×複数モデルでは応答まで時間がかかる |
| 評価履歴の前処理情報はサマリーのみ | localStorage履歴へは `pre: {source, summary}`（由来と1行サマリー）を保存する。しきい値等の全パラメータはサマリー文字列に含まれる範囲のみで、旧形式の履歴は「未記録」表示になる |

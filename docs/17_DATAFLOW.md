...
| CER評価（主指標） | 「評価APIが画像ごとに `levenshtein_ops`（DP+バックトレースの純Python実装）で編集距離とアラインメント（置換/脱落/挿入）を算出 → **CER=全画像の編集距離総和÷全画像の正解文字数総和（マイクロ平均。画像ごとのCER平均は使わない）** → 文字正解率=1-CER。混同集計はアラインメント操作をCounterで集計しTOP10を応答へ。comparisonでは画像単位の編集距離を学習前と比較して改善/同等/悪化を判定し、matchの遷移から完全一致へ改善/から悪化を集計。CER差=学習後-学習前（負=改善）・相対改善率=(学習前-学習後)/学習前」。比較はcase-sensitive・trimのみ（完全一致評価と同じ正規化）。Accuracy（完全一致率）は業務指標として全画面で併記 | `ocr_evaluation.py`（`levenshtein_ops` / `evaluate_ocr`） |
| モデル比較（CER中心） | 「最新評価履歴から `buildModelComparison`（9指標×モデルの値と最良値。CER/悪化件数=最小が最良・他=最大が最良）→ `buildWinLoss`（指標ごとに単独最良へ1勝。タイ・欠損は勝者なし）→ `recommendModel`（勝利数最多→タイはCER最小→文字正解率最大。**Accuracy単独では決めない**。理由=最良を取った指標一覧）→ `buildConfusionComparison`（全モデルの混同を件数合計で統合しモデル別比較。旧形式=混同データなしはnull表示）」。比較テーブルは各行の最良値を緑ハイライト | `lib/modelCompare.js` / `ModelsView.jsx` |
| モデルカルテ（モデル管理画面） | 「評価実行時にフロントが localStorage 履歴（`ocr_model_eval_history_by_project_v1`）のエントリへ評価APIの応答値（`correct`→correct_count / `total`→total_count / `mismatch_count`→misrecognized_count / comparison→improvement_rate（%）・improvement_count / データセット名・whitelistモード・pre）を追記保存 → モデル管理画面が `lib/modelEval.js` で正規化し、モデル詳細に最新評価（認識率・正解/総数・誤認識・学習前比の改善率/改善件数）・評価条件（データセット/画像数/前処理/Whitelist/日時）・評価履歴表を表示」。改善率=comparison.improvement_rate×100（学習前比・%）、改善件数=comparison.correct_delta（正解数の学習前との差分）。推奨バッジは履歴から自動判定（🟢=最新評価の最高Accuracy / 🏆=全履歴最高 / 🔴=eng系ベース名）。旧形式エントリ（percent/atのみ）は欠損をnull化し「未記録」表示（後方互換・エラーにしない）。モデルサイズは `/models/info` の `model_size_mb`（サーバーがモデル実体をstat） | `lib/modelEval.js` / `ModelsView.jsx` / `model_registry.py`（`list_model_infos`） |

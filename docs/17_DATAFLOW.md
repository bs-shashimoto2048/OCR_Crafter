...
| Step5のOCR実行タイミング（性能設計） | 「画像切替・回転・前処理変更・OCR設定変更 → **OCRは実行せず**、①中間・最終画像プレビューだけを `/api/ocr/preview-file/batch`（`slots_json=[]`）で非同期更新（300msデバウンス・AbortControllerで旧リクエスト中止）②OCR候補は実行条件キー（画像key+回転+Step5前処理+有効スロット設定）のキャッシュを引き、ミスなら『要再実行』表示 → ユーザーが[OCR再実行]を押したときだけ有効スロットを1リクエストのバッチで推論（サーバー側で前処理1回＋スロット順に逐次実行=同時実行数1）」。自動実行はStep5専用設定（既定OFF）で、ONでも連続操作終了後に1回だけ。結果キャッシュは二層（サーバー=処理済み画像sha256+設定のLRU128件・エラー除外 / フロント=実行条件キーのLRU30件）で、同一条件へ戻ると即表示・同じOCRを再計算しない。editing_stateへはOCR候補・base64画像・ローディング状態を保存せず、差分がない場合は書き込まない | `main.py`（`run_preview_ocr_batch` / `_predict_preview_slot`）/ `services/ocr_preview_cache.py` / `lib/evalOcrRun.js` |

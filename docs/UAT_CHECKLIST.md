...
| 16 | Benchmark Runner | モデルあり | Benchmark Runnerを実行 | BM-IDつきのLeaderboardが表示される | | |
...
| 31 | Dataset Manager | 学習データセット作成済み | Dataset Managerで一覧・詳細・Dataset⇔Model双方向リンクを確認 | DS-IDつきのDataset詳細に前処理Version/Hash・使用モデル一覧が表示される | | |
| 32 | Benchmark Center | Dataset/Experiment/モデルあり | Benchmark Centerで既存の評価結果を横断比較 | 比較表・最良値ハイライトが表示される（新たな評価は実行されない） | | |
| 33 | 推論モデル切替 | モデル2件以上 | モデル管理で「推論に使用」を別モデルへ切替 | 保存され、ページ再読み込み後も選択が維持される。切替済みモデル自身のボタンのみ「推論使用中」で無効になる | | |
...
5画面（ジョブ管理/Benchmark Runner/監査ログ/システム状態/リリース管理）× 4解像度（1920×1080 / 1440×900 / 1366×768 / 900×1400）で横スクロールなし・空状態表示・縦積み切替を確認（docs/15 2026-07-23参照。当時「Benchmark」は現在の「Benchmark Runner」）。大量ケース表はページング（50件/ページ）・一覧はコンテナ内スクロール。

**未実施（本チェックリスト作成後に追加された画面）**: Dataset Manager・Benchmark Centerは2026-07-28に追加された画面のため、この解像度確認には含まれていない。同様の解像度確認を別途実施のこと。

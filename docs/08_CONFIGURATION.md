...
| `ocr_training_settings_tab_v1` | 学習画面「次回学習の設定」（インライン編集）で最後に開いた設定カテゴリ（`training-settings`/`preprocess`/`augmentation`/`engine`の4タブ。初期表示の維持用・設定値は含まない）。**旧タブ構成時代の値（`data-split`/`training-params`）は`training-settings`へ安全に移行**され、想定外の値も同様にフォールバックする（`lib/trainingSettingsTabs.js`）。`preprocess`（学習前処理タブ）はv1.0.0で追加（既存の保存値には影響しない） | 全体共通 |

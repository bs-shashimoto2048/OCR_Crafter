...
| `ocr_eval_preview_slots_by_project_v1` | Step5（評価用データ作成）専用のOCR設定・最大3モデル（`{slots: [{enabled, engine, paddleModel, tesseractModel, easyocrLangs, includeLowercase, psm, whitelist}]}`。初期=モデル1のみ有効。ラベル編集の推論設定とは独立） | プロジェクト別 |

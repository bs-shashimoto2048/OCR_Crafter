...
- **制約（明記）**:
  ...
  - `outputs/app.db`（Job System Aの学習Job実行履歴）・`data/jobs/job_manager.db`・`data/model_ids.json`・`data/dataset_ids.json`・`data/audit/`はプロジェクト単位のバックアップ対象外です（詳細: [25_DISASTER_RECOVERY.md](25_DISASTER_RECOVERY.md) §4）。`outputs/app.db`・`data/jobs/job_manager.db`は`services/sqlite_backup.py`（Issue #147、`sqlite3.Connection.backup()`使用・UI/APIなし）でBackend停止不要のsnapshotを作成できます。**復元**は同モジュールの`restore_app_db()`/`restore_job_manager_db()`（Issue #162、Backend停止済みであることが前提）で行えます。復元先に古い`-wal`/`-shm`が残っていると単純なファイル置換では復元データが見えないsilent failureになることが実証されているため、両関数は置換前にそれらを削除し、失敗時は自動ロールバックします。手順の詳細（preflight・Backend停止・WAL/SHM handling・post-restore verification）は[SQLITE_GLOBAL_DB_RESTORE_RUNBOOK_162.md](workitems/reliability/SQLITE_GLOBAL_DB_RESTORE_RUNBOOK_162.md)を参照してください

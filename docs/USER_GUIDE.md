...
- **ボタン**: 「新規プロジェクト」（テンプレート選択モーダル）/ カードの「開く」「学習」「評価」「Benchmark」「Report」・「・・・」メニューの「削除」
...
- **推論使用中の強調表示**（新規）: 「推論に使用」に設定したモデルの行は、一覧上で常時（hoverしなくても）分かるようにシアン系の背景・左端のライン・「推論使用中」バッジ・管理Noの強調で表示されます。比較用のチェックボックスとは独立しており、両方を同時に選択できます。「推論に使用」ボタンを押すと即座に表示が切り替わります（この選択はブラウザのセッション内でのみ保持され、ページを再読み込みすると既定値へ戻ります）

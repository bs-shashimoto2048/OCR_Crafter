...
一覧の各行はサムネイル付きの2段組で、プロジェクトの状態・品質・運用状況が一目で分かるようになっています。

- **1行目**: ★（選択中のみ）+ プロジェクト名 / 作成元テンプレート / 状態バッジ / Productionモデル（管理No）
- **2行目**: 画像数 / ラベル数（完了・%）/ モデル数 / Benchmark件数 / Best CER / 進捗バー（%＋現在の工程名）

**状態バッジ**: 🟢使用中（選択中）/ 🟡学習中・🔵評価中（学習/評価Jobが実行中）/ ⚪Archived（Productionが無く全モデルがArchived）/ 「—」（該当なし）。

**Production**: Productionモデルが存在する場合のみ管理No（例: `M0009`）を表示。存在しなければ「—」です。

**Best CER**: 記録されている評価結果のうち、①Production→②Candidate→③プロジェクト内で最良のモデルの順に採用して表示します（推測での補完は行いません。評価未実施のプロジェクトは常に「—」）。

**クイックアクション**（行右端のアイコン）: 開く / 学習（データ作成・学習画面）/ 評価（モデル評価画面）/ Benchmark / レポート / 削除。評価・レポートはモデルが1件も無い場合、Benchmarkは画像が1枚も無い場合に無効化されます（ボタンにカーソルを合わせると理由が表示されます）。

**行クリック**: 操作ボタン以外の行のどこをクリックしてもプロジェクトが開きます（キーボードはTabで行にフォーカスし、Enter/Spaceでも開けます）。

**ソート・検索**: 列見出し（更新日時・画像・ラベル・モデル・Best CER・進捗）をクリックすると並び替えできます（再クリックで降順→昇順→既定の並びの順に切り替え）。検索欄はプロジェクト名に加え、テンプレート名・Productionモデル・状態でも絞り込めます。

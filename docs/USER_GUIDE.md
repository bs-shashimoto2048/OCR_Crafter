...
- **ヘッダー**: サムネイル（大きめ表示。プロジェクトの見た目上の識別子）＋プロジェクト名＋状態バッジ＋テンプレート名。Productionモデルがあれば「Production」＋管理No（例: `M0009`）も表示。右上の「・・・」から削除できます（誤操作防止のためクイックアクションとは別メニューにしています）
- **品質情報**: 「データ」（画像数・ラベル数・モデル数）、「品質」（Best CER・Exact Match）、「性能」（Balance Score・P95推論時間・実施回数）の3グループで表示します。Exact Matchは記録が無い場合は表示されません（推測での補完は行いません）。性能グループはBenchmarkを1度も実施していない場合、単なる「—」ではなく「未実施」と表示します
- **進捗**: 進捗バー（%）＋現在の工程名。工程が特定できない場合は「—」と表示します
- **クイックアクション**: 上段=「開く」「学習」、下段=「評価」「Benchmark」「Report」。評価・Reportはモデルが1件も無い場合、Benchmarkは画像が1枚も無い場合に無効化されます（ボタンにカーソルを合わせると理由が表示されます）。Benchmarkボタンには最新結果（例:「最新Benchmark: Balance 96.2 / P95 42 ms」）または「Benchmarkはまだ実施されていません」がツールチップで表示されます
...
**Production**: Productionモデルが存在する場合のみ管理No（例: `M0009`）を表示。存在しなければ表示しません。
...
**Health Badge**: プロジェクトの健全性をルールベースで判定します（AIによる推定は行いません）。バッジにカーソルを合わせると、判定根拠（例:「評価済みモデルがあります。」「Benchmarkは未実施です。」）がツールチップで表示されます。

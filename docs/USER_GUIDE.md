...
- **品質情報**: 画像数・ラベル数・モデル数（左列）、Benchmark件数・Best CER・Exact Match（右列）。Exact Matchは記録が無い場合は表示されません（推測での補完は行いません）
- **進捗**: 進捗バー（%）＋現在の工程名。工程が特定できない場合は「—」と表示します
- **クイックアクション**: 上段=「開く」「学習」、下段=「評価」「Benchmark」「Report」。評価・Reportはモデルが1件も無い場合、Benchmarkは画像が1枚も無い場合に無効化されます（ボタンにカーソルを合わせると理由が表示されます）
- **フッター**: 最終更新日時（相対表示＋絶対表示）／Health Badge
...
**Health Badge**: プロジェクトの健全性をルールベースで判定します（AIによる推定は行いません）。

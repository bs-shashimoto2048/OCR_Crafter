...
学習画面左の「次回学習の設定」はカテゴリサマリー表示です。カードを選択すると**モーダルではなく左パネルがそのまま編集表示へ切り替わり**（左パネルが広がり、右側の学習状況は表示されたまま）、**3タブ**（学習設定 / オーグメンテーション / エンジン設定）で編集できます。**ここで変更した設定は次回の学習から適用され、完了済みの学習結果には影響しません。**

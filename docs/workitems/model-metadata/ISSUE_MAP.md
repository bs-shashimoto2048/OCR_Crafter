...
| 8 | Training Metadata Factory | 7 | `create_from_training(...)`。学習完了時に`ModelMetadata`を生成（保存は行わない） | 着手中（未作成→今回作成） |

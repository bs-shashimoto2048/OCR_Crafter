...
| 9 | Models API・Models画面連携 | 7, 8 | Models API（Facade、`ModelsAPI`）実装完了・PRレビュー待ち（[#44](https://github.com/bs-shashimoto2048/OCR_Crafter/issues/44)、詳細は[MODELS_API_DESIGN_NOTES.md](MODELS_API_DESIGN_NOTES.md)）。`/models/info`の実際のConsumer切替（レスポンス形式維持）は別途Issue化する | 実装完了・PRレビュー待ち（[#44](https://github.com/bs-shashimoto2048/OCR_Crafter/issues/44)）。Consumer切替は未作成 |

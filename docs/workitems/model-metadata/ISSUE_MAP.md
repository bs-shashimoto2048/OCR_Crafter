...
| 14 | TrOCR Legacy Metadata Adapter Compatibility | 4, 5, 7 | `TrOCRMetadataAdapter`新設・`MetadataReader`/`ModelCatalog`への`.trocr.json`分岐追加（既存2形式と同型・schema変更なし） | **Implemented, PR review pending**（[#110](https://github.com/bs-shashimoto2048/OCR_Crafter/issues/110)。詳細は[TROCR_LEGACY_METADATA_ADAPTER_110.md](TROCR_LEGACY_METADATA_ADAPTER_110.md)参照） |
...
**（2026-08-18更新、Issue #110）**: #14（TrOCR Legacy Metadata Adapter Compatibility）を実装済み（Implemented, PR review pending）。Consumer Migration（#9-#13）は引き続き着手しない方針のまま。

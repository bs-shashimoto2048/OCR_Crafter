...
| 8 | Training Metadata Factory | 7 | `create_from_training(...)`。学習完了時に`ModelMetadata`を生成（保存は行わない）。詳細は[TRAINING_METADATA_FACTORY_DESIGN_NOTES.md](TRAINING_METADATA_FACTORY_DESIGN_NOTES.md) | 実装完了・PRレビュー待ち（[#42](https://github.com/bs-shashimoto2048/OCR_Crafter/issues/42)） |

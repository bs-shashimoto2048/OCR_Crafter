...
| 14 | TrOCR Legacy Metadata Adapter Compatibility | 4, 5, 7 | `TrocrMetadataAdapter`新設・`MetadataReader`/`ModelCatalog`への`.trocr.json`分岐追加（既存2形式と同型・schema変更不要） | **推奨**（Investigation #108で唯一推奨した次Issue。詳細は[MODEL_METADATA_CONSUMER_MIGRATION_REASSESSMENT_108.md](MODEL_METADATA_CONSUMER_MIGRATION_REASSESSMENT_108.md)参照） |

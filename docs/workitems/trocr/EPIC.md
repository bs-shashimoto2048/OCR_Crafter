...
次フェーズ: TrOCR Backend推論コアPRのレビュー・マージ後、[ISSUE_MAP.md](ISSUE_MAP.md)の確定順序で残りの実装Issue（Engine判定既存バグ修正のFrontend側、TrOCR Training/Inference/Evaluation等）を作成予定。
...
- [ ] #16（Feature: TrOCR Backend単画像推論コア実装、実装済み・PRレビュー待ち）

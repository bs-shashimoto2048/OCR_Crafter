...
現状の実際の値（`engine_capability.py`のBuiltin Capability）: Backend既定Registry登録済みの4エンジン（`tesseract`/`paddleocr`/`easyocr`/`trocr`）はいずれも`supports_evaluation=True`（**Issue #73（PaddleOCR Evaluation Predictor）でpaddleocrをFalse→Trueへ、Issue #75（EasyOCR Evaluation Predictor）でeasyocrをFalse→Trueへ、Issue #77（TrOCR Evaluation Predictor）でtrocrをFalse→Trueへそれぞれ変更済み**。Issue #67時点はtesseractのみTrueだった）。「登録済みだがUnsupported」なEngineの実例はもう存在しない（`custom`はRegistry未登録のためUnknownのまま）。
...
| trocr     |        yes |                true | 登録済Predictorを返す（Issue #77でTrueへ変更） |

...
- common: データセットID / データセット内容Hash（画像名+正解文字列のsha256） / 画像数 / ラベル数 / 文字正規化バージョン（`trim+NFC`） / CERバージョン（`cer-v1-micro`） / **前処理識別子（`<mode>:<実効前処理Hash>`。none時は `none`=従来と同一Hash・後方互換）**
...
`preprocess: {mode, settings?, model?}` で指定（未指定=none=従来動作）。

| mode | 内容 | 実効Hash |
|---|---|---|
| `none` | 元画像のまま | -（識別子 `none`） |
| `manual` | グレースケール・二値化の手動設定（評価と同じ `apply_eval_preprocess`） | 正規化設定JSONのsha256 |
| `training` | 指定モデルの学習時前処理スナップショット（未記録はエラー・推測しない） | `training_preprocess_hash` |
| `project` | プロジェクトの現在の前処理スナップショット（未実行はエラー） | スナップショットの前処理Hash |

- **前処理済み画像はBenchmark開始時に一度だけ生成**（Job ID付き一時ディレクトリ）し、**全エンジンへ同じ最終入力画像を渡す**（エンジンごとの再処理なし）
- 実効前処理HashはProfile Hashへ含まれる（前処理が違えば別条件のBenchmarkとして区別）
- 成功時は一時画像を削除。失敗時はJob ID付き一時ディレクトリのまま残し調査に使える
- 実行結果へ `preprocess: {mode, hash, source_model}` を保存し、Leaderboard画面へ表示

...
import { buildOcrRunKey, createLruCache, shouldAutoRunOcr, shouldPrefetchNext } from "../src/lib/evalOcrRun.js";
...
test("shouldPrefetchNext: 画像移動済み・OCR実行中・キャッシュ済み・次画像なしでは先読みしない", () => {
  const base = { autoRun: true, stillOnSameImage: true, ocrRunning: false, hasNextItem: true, nextCached: false };
  assert.equal(shouldPrefetchNext(base), true);
  assert.equal(shouldPrefetchNext({ ...base, stillOnSameImage: false }), false); // 前へ/次へ連打中
  assert.equal(shouldPrefetchNext({ ...base, ocrRunning: true }), false); // 現在画像OCRと同時に先読みしない
  assert.equal(shouldPrefetchNext({ ...base, nextCached: true }), false); // キャッシュ済みは不要
  assert.equal(shouldPrefetchNext({ ...base, hasNextItem: false }), false);
  assert.equal(shouldPrefetchNext({ ...base, autoRun: false }), false);
});

...
test("Production表示: 存在すればカード右上へ管理Noを独立表示、存在しなければ「Productionなし」と控えめに表示する", () => {
  const html = render();
  assert.ok(html.includes("M0009"));
  // 検索欄のプレースホルダーにも「Production」の語が含まれるため、カード右上の専用ブロックで判定する
  const productionBlockMarker = 'text-emerald-300">Production</p>';
  assert.ok(html.includes(productionBlockMarker));
  assert.ok(html.includes("Productionなし")); // cursive/empty_projはProductionなし
  const soloHtml = render({
    projects: ["cursive"],
    projectSummaries: { cursive: baseProps().projectSummaries.cursive },
  });
  assert.ok(!soloHtml.includes(productionBlockMarker));
  assert.ok(soloHtml.includes("Productionなし"));
});
...
test("Progress表示: 現在の工程名をバー上部に強調表示し、%を表示する", () => {
  const html = render();
  assert.ok(html.includes("評価")); // tube_20260710の現在の工程
  assert.ok(html.includes("98%")); // (1+1+0.9+1)/4=97.5%→98%
});
...
test("クイックアクション: Primary（開く）はSecondary（学習/評価/Benchmark/Report）と異なる強調スタイルを持つ", () => {
  const html = render();
  const openButtonMatch = html.match(/<button[^>]*aria-label="tube_20260710 を開くへ"[^>]*>/);
  assert.ok(openButtonMatch, "open button should exist");
  assert.ok(openButtonMatch[0].includes("bg-accent"), "primary action should use the filled accent style");
  const trainButtonMatch = html.match(/<button[^>]*aria-label="tube_20260710 を学習へ"[^>]*>/);
  assert.ok(trainButtonMatch, "train button should exist");
  assert.ok(!trainButtonMatch[0].includes("bg-accent"), "secondary actions should not use the primary filled style");
});

test("Benchmark未実施時: 「Benchmark未実施」＋実行を促すリンクを表示する（単なる未実施表示のみにしない）", () => {
  const html = render();
  assert.ok(html.includes("Benchmark未実施"));
  assert.ok(html.includes("Benchmarkを実行"));
});

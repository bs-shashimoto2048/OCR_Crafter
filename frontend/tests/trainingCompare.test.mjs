...
test("normalizeTrainingCondition: モデル情報から学習条件を正規化・表示行は未記録フォールバック", () => {
  ...
  // 表示行: 実験名〜学習メモの17項目（分割比率/Seed/分割方式/Augプリセット/設定/生成枚数を含む）
  assert.equal(TRAINING_CONDITION_ROWS.length, 17);
  ...
  // 分割・Augの新フィールド（m1は未記録）
  assert.equal(rowOf("splitSeed").value(cond), "未記録");
  assert.equal(rowOf("splitMethod").value(cond), "未記録");
  assert.equal(rowOf("augPreset").value(cond), "未記録");
  assert.equal(rowOf("augGenerated").value(cond), "未記録");
  // 旧モデル（情報なし）も安全に未記録表示
...
test("normalizeTrainingCondition: 分割・オーグメンテーションの新フィールド（記録あり/なし）", () => {
  const rowOf = (key) => TRAINING_CONDITION_ROWS.find((r) => r.key === key);
  const cond = normalizeTrainingCondition({
    dataset_split_ratio: { train: 0.85, val: 0.1, test: 0.05 },
    split_seed: 42,
    split_method: "image",
    augmentation_config: {
      preset: "weak",
      multiplier: 1.5,
      rotation: { enabled: true, max_degrees: 2, probability: 0.3 },
      brightness: { enabled: true, range: 0.1, probability: 0.3 },
      contrast: { enabled: false },
      blur: { enabled: true, strength: "weak", probability: 0.1 },
      noise: { enabled: false },
    },
    augmentation_generated: 8,
  });
  assert.equal(rowOf("splitRatio").value(cond), "0.85 / 0.10 / 0.05");
  assert.equal(rowOf("splitSeed").value(cond), "42");
  assert.equal(rowOf("splitMethod").value(cond), "画像単位");
  assert.equal(rowOf("augPreset").value(cond), "弱い");
  assert.ok(rowOf("augmentation").value(cond).includes("回転±2°"));
  assert.equal(rowOf("augGenerated").value(cond), "8枚");
  // Seed=0 / 生成0枚 は未記録に化けない（Number(null)===0の罠と区別）
  const zero = normalizeTrainingCondition({ split_seed: 0, augmentation_generated: 0 });
  assert.equal(rowOf("splitSeed").value(zero), "0");
  assert.equal(rowOf("augGenerated").value(zero), "0枚");
  // 旧形式（use_augmentation由来）はON/OFF表示・プリセットは旧形式表記
  const legacy = normalizeTrainingCondition({ ocr_augmentation: { enabled: true, strength: 2 } });
  assert.equal(rowOf("augmentation").value(legacy), "ON（強度 2）");
  assert.equal(rowOf("augPreset").value(legacy), "旧形式（強度指定）");
});
